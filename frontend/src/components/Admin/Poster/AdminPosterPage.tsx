...
const AdminPosterPage = () => {
  ...
  const fetchPosters = useCallback(async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Lấy danh sách poster thất bại');
      toast.error(err.response?.data?.message || 'Lấy danh sách poster thất bại', {
        position: "top-right",
        autoClose: 3000,
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error(err.response?.data?.message || 'Xóa poster thất bại', {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,

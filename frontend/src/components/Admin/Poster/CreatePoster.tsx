...
import posterService, { CreatePosterData } from '../../../services/admin/poster';
...
const CreatePoster = ({ token, onSuccess, onCancel }: CreatePosterProps) => {
  const [formData, setFormData] = useState<CreatePosterData>({
    title: '',
    image_url: '',
    status: 'active',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newPoster = await posterService.createPoster(token, formData);
      onSuccess(newPoster);
      toast.success('Tạo poster mới thành công!', {
        position: "top-right",
        autoClose: 3000,
      ...
      setError(err.response?.data?.message || 'Tạo poster thất bại');
      toast.error(err.response?.data?.message || 'Tạo poster thất bại', {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              disabled={loading}
            >
              <option value="active" className="text-green-800 bg-green-50">Hoạt động</option>
              <option value="inactive" className="text-red-800 bg-red-50">Không hoạt động</option>
            </select>
          </div>

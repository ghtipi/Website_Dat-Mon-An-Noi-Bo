...
const EditPoster = ({ token, poster, onSuccess, onCancel }: EditPosterProps) => {
  ...
  const [previewImage, setPreviewImage] = useState<string | null>(poster.image_url);
  ...
  useEffect(() => {
    ...
    setPreviewImage(poster.image_url);
  }, [poster]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Cập nhật poster thất bại');
      toast.error(err.response?.data?.message || 'Cập nhật poster thất bại', {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
